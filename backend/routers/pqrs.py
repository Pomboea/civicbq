...
def to_response(p: Pqr) -> PqrResponse:
    return PqrResponse(id=p.id, titulo=p.titulo, categoria=p.categoria.value, descripcion=p.descripcion,
...
def list_pqrs(estado: Optional[str] = Query(None), categoria: Optional[str] = Query(None),
    ...
    if current_user.role.value == "ciudadano":
        q = q.filter(Pqr.creado_por == current_user.id)
    if estado:
        q = q.filter(Pqr.estado == to_enum(estado, PqrStatus))
    if categoria:
        q = q.filter(Pqr.categoria == to_enum(categoria, PqrCategory))
    return [to_response(p) for p in q.order_by(Pqr.created_at.desc()).all()]
